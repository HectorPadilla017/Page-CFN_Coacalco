...
export const getChannelAction = () => async (dispatch, getState) => {
  try {
    ...
    const res = await axios.get(
      `https://www.googleapis.com/youtube/v3/search?key=${Key}&channelId=${Channel}&fields=items(id(videoId),snippet(title,thumbnails))&part=snippet,id&order=date&maxResults=${num}`
    );
    dispatch({
